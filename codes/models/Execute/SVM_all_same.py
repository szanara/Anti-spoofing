...
path2save_im = '/home/cz/mds3/visao/figures/TrainTest_same/SVM/same'
...
for descriptor_train, descriptor_dev, descriptor_eval in zip(trains, develops, evaluations):
    ...
    X_test = np.load(path_develop+f'{descriptor_eval}/X_train.npy', allow_pickle=True)
    y_test =np.load(path_develop+f'{descriptor_eval}/y_train.npy', allow_pickle=True)
    ...
    svm = SvmModel(X_train = X_train,
               ...
               X_dev = X_develop,
               y_dev = y_develop)
    FAR_test, FRR_test, HTER, EER, training_time = svm.SVM(plot_roc=[True,image_save],
                                                      plot_Testconfusion=[True,image_save])
    
    results.append([descriptor_train, descriptor_dev, descriptor_eval, FAR_test, FRR_test, HTER, EER, training_time])
# Convertendo a lista para um DataFrame do pandas
results_df = pd.DataFrame(results, columns=['descriptor_train', 'descriptor_dev', 'descriptor_eval', 'FAR_test', 'FRR_test', 'HTER', 'EER','training_time'])

# Salvando o DataFrame como um arquivo CSV
results_df.to_csv(path2save+'/results_TrainTest_same_SVM.csv', index=False)
